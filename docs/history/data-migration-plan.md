...
- 来源只提供当前帖子时，把当前内容写入 `post_histories.revision=1`，并令主表
  `current_revision=1`。只有来源能提供确凿旧版本及其顺序时，才在当前版之前按证据迁入。
...
- 来源只提供当前评论时把当前正文写入 `comment_histories.revision=1`，并令主表指针为 1；
...
6. 没有来源旧版本证据的帖子和评论恰有一条当前版 `revision=1`，且主表指针为 1；
...
- [ ] 确认旧停用用户迁入后仍被封禁。
- [ ] 确认每个当前帖子/评论都有历史真源与有效指针；若迁入真实旧版本，逐行核对来源证据与顺序。
