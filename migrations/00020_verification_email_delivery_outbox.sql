...
COMMENT ON COLUMN email_verification_codes.purpose IS '验证码用途：registration 注册验证，password_reset 密码重置；不同用途不可混用。';
COMMENT ON COLUMN email_verification_codes.last_sent_at IS '最近一次发送申请被持久化受理的时间，用于冷却；不代表供应商实际发送成功。';
