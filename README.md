...
业务指标同样只使用固定枚举 label：审核提交、供应商失败、事务提交后的终态、回调鉴权/载荷/处理结果，以及验证码真实发信、供应商失败、邮箱配额限流和服务器在途拒绝。待复核队列采用单刷新者缓存：同一时刻至多一个 scrape 通过只读短事务刷新管理端 `queue_items` 的真实分组口径，其余并发 scrape 不等待、不再占数据库连接，直接导出最近成功值。`cache_ready`、最后刷新是否成功、刷新中状态、错误总数和最后成功时间共同标明缓存新鲜度；启动尚无成功值时不导出队列深度，刷新失败也不会让整个 `/metrics` 返回 500。指标绝不记录用户 ID、图片对象键、供应商任务号、邮箱、验证码、正文或错误文本。预供应商阶段发生的验证码限流与在途拒绝固定使用 `provider="none"`。
...
| 21 | 用户名人工复核候选快照与提交版本 |
| 22 | 用户名内部空格支持及统一空格规范化约束 |

升级前先备份并检查全部既有用户（含已注销用户）的用户名：按应用的 NFKC、去首尾空白、
内部连续空格折叠、Unicode 字母/数字/下划线/普通空格、2–24 字符及保留名称规则校验，再按数据库不区分大小写的规则
...
用户名成功修改后需间隔满 30 天（连续 720 小时）才能再次修改；注册、无实际变化和失败请求不计次。
冷却期内返回 429 `username_change_limited`，不按自然月重置；不影响其他资料更新。
升级后的数据库注释由迁移 18–22 同步维护，已发布迁移 1–17 保持历史版本语义。

迁移 18 尚未发布，字符集合已同步允许普通空格，使版本 17 的含空格名称可以进入升级链路；迁移 22 也更新同一字符函数，支持已有版本 21 的开发库升级。所有存量名称须先规范化并检查空名、重名与保留名称；不自动替账号分配新身份。注册、改名、用户名登录统一使用空格折叠后的值，数据库仅接受规范化存储值。
