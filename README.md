...
注册和密码重置验证码采用同步发送。请求最多等待邮件供应商 10 秒，失败或超时返回 503，
前端展示错误，由用户重新申请；不需要邮件投递任务或后台补发。成功表示供应商已受理且
数据库事务已提交，不保证收件箱已经收到邮件。供应商受理后数据库提交失败时，请求仍失败。

`email_verification_codes.code` 保存六位验证码明文，`code_digest` 保存校验摘要；
`EMAIL_VERIFICATION_SECRET` 仅用于摘要计算。发送失败回滚本次更新，保留此前成功发送
且仍有效的验证码；成功重发替换原值，消费或错误次数耗尽时清空明文。过期立即禁止校验，
旧明文在后续状态写入时清理，没有定时清理任务。API、日志、追踪和指标不输出验证码。
