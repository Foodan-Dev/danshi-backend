...
- **类别**：新增端点，非破坏。
- **新增端点**：
  - `POST /api/v2/auth/password-reset-codes`：按邮箱请求重置验证码；正常成功响应一致，供应商失败或超时明确返回 503。
  - `POST /api/v2/auth/password-resets`：提交邮箱、验证码与新密码，成功后撤销全部既有会话但不自动登录。
- **安全语义**：重置验证码与注册验证码按用途隔离，验证码记录同时保存明文和校验摘要；限时且仅可消费一次。
...
注册和密码重置验证码同步发送：成功表示供应商已受理且验证码记录已提交；供应商失败或
超时返回 503，前端直接展示错误并允许重试，不进入后台补发。发送失败回滚本次更新，
此前成功发送且仍有效的验证码保持可用。正常成功响应和限流处理不直接说明账号状态，
但同步发送的耗时及失败状态可能产生差异，不承诺完整防枚举。

验证码记录同时保存六位明文和校验摘要；消费或错误次数耗尽时清空，成功重发时替换。
有效期仍是十分钟，过期立即禁止校验；旧明文在后续状态写入时清理。HTTP 不返回验证码。
