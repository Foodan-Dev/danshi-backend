...
| `POST /api/v2/auth/login` | `email` 登录 | `identifier` 可为 email 或 username；服务端暂兼容旧 `email` 字段 |

已占用 username 返回 409 `username_taken`。改名后的历史 username 继续归原账号占用，不会分配给其他账号。

**前端影响清单**：

- [ ] 注册页将 `username` 设为必填，并在服务端返回 `username_taken` 时提示用户更换。
- [ ] 登录请求迁移到 `identifier`；现有仅提交 `email` 的客户端在过渡期仍可使用。
- [ ] 不把 username 当作可随意复用的昵称；修改后旧 username 不能用于新注册。
...
### §3.2：新增 username 变更历史查询端点

- **类别**：新增端点，非破坏。
- **动机**：username 是公开身份标识，改名必须保留不可篡改的占用与审计记录；本人和具备用户管理能力的管理员需要能够查询该记录。
- **新增端点**：`GET /api/v2/users/{user_id}/username-history`。
- **可见性**：该端点仅用户本人可读；管理员通过用户取证详情接口查询，普通用户不能读取他人历史。
- **返回语义**：按发生时间倒序返回旧 username、新 username 与变更时间，不返回内部审核供应商信息。
- **前端影响**：账号设置页可展示本人 username 变更历史；管理员取证页按权限展示指定用户的历史记录。
