...
### BC-021 / name 成为注册必填的全局身份标识

**动机**：`users.name` 不再只是可重复的展示昵称，而是用户在全平台可识别、可用于登录的公开身份。

| 项目 | 旧契约 | 新契约 |
|---|---|---|
| `POST /api/v2/auth/register` 的 `name` | 可省略、可重复 | 必填；规范化后全局唯一 |
| 可接受字符 | 未定义 | 2–24 个字符；文字、数字或下划线；允许中文 |
| 重复判定 | 无 | Unicode NFKC 规范化后，大小写不敏感 |
| `POST /api/v2/auth/login` | `email` 登录 | `identifier` 可为 email 或 name；服务端暂兼容旧 `email` 字段 |

已占用 name 返回 409 `name_taken`。改名后的历史 name 继续归原账号占用，不会分配给其他账号。

**前端影响清单**：

- [ ] 注册页将 `name` 设为必填，并在服务端返回 `name_taken` 时提示用户更换。
- [ ] 登录请求迁移到 `identifier`；现有仅提交 `email` 的客户端在过渡期仍可使用。
- [ ] 不把 name 当作可随意复用的昵称；修改后旧 name 不能用于新注册。

## 新增、勘误与行为变更（不计入上述破坏性变更）

### §3.2：新增 name 变更历史查询端点

- **类别**：新增端点，非破坏。
- **动机**：name 是公开身份标识，改名必须保留不可篡改的占用与审计记录；本人和具备用户管理能力的管理员需要能够查询该记录。
- **新增端点**：`GET /api/v2/users/{user_id}/name-history`。
- **可见性**：仅用户本人或具备用户管理能力的管理员可读；普通用户不能借此枚举他人的身份变更历史。
- **返回语义**：按发生时间倒序返回旧 name、新 name 与变更时间，不返回内部审核供应商信息。
- **前端影响**：账号设置页可展示本人 name 变更历史；管理员取证页按权限展示指定用户的历史记录。
...
- **安全语义**：重置验证码与注册验证码按用途隔离，均只保存摘要、限时且仅可消费一次。
