...
> 此处记录的是当时的迁移语义；历史存储与审核版本锚定随后由 BC-020 再次调整。
...
### BC-020 / 编辑历史改为全版本表并增加当前指针

**动机**：旧模型只把被替换内容写进历史，回退时不得不再次写入相同快照，造成同一内容以
不同 revision 重复存储。全版本模型让历史成为版本真源，主表只保留查询副本和当前指针。

| 项目 | 旧契约 | 新契约 |
|---|---|---|
| 创建帖子/评论后的 history | 空数组 | 包含当前 `revision=1` |
| 普通编辑 | 保存被替换版，当前内容只在主表 | 追加编辑后的新版本并推进指针 |
| 历史回退 | 复制旧快照为新 revision，并合成审核流水 | 只移动指针、同步主表副本，不追加版本或审核流水 |
| 删除 | 可能追加删除前快照 | 只设置软删除字段，不追加版本 |
| history 单项 | 不标识当前版 | 新增 `is_current: boolean` |
| 审核版本绑定 | 整数版本号没有历史外键 | `(对象 id, content_revision)` 复合外键锚定历史版本 |

回退到已有审核结论的版本时直接复用该结论；没有结论时才按普通编辑重新送审。回退后的
`current_revision` 可以小于最大 revision，后续编辑仍从历史最大 revision 的下一个值继续追加。

**前端影响清单**：

- [ ] 历史页接受创建后即有 `revision=1`，不要再用空数组判断“从未编辑”。
- [ ] 使用 `is_current` 标识当前指向版本；不要假定列表第一项永远是当前版本。
- [ ] “已编辑”继续使用主体响应的 `is_edited`，不要以历史列表是否非空推导。
- [ ] 回退成功后刷新主体与历史列表；同一 revision 的审核展示沿用原结论。

### BC-021 / 用户名成为注册必填的全局身份标识

**动机**：`users.name` 不再只是可重复的展示昵称，而是用户在全平台可识别、可用于登录的公开身份。

| 项目 | 旧契约 | 新契约 |
|---|---|---|
| 注册用户名字段 | `name` 可省略、可重复 | `username` 必填；规范化后全局唯一 |
| 可接受字符 | 未定义 | 2–24 个字符；文字、数字、下划线或内部普通空格；允许中文，连续空格折叠为一个 |
| 重复判定 | 无 | Unicode NFKC 规范化后，大小写不敏感 |
| `POST /api/v2/auth/login` | `email` 登录 | `identifier` 可为邮箱或用户名；服务端暂兼容旧 `email` 字段 |

已占用用户名返回 409 `username_taken`。改名后的历史用户名继续归原账号占用，不会分配给其他账号。

**前端影响清单**：

- [ ] 注册页将 `username` 设为必填，并在服务端返回 `username_taken` 时提示用户更换。
- [ ] 登录请求迁移到 `identifier`；现有仅提交 `email` 的客户端在过渡期仍可使用。
- [ ] 不把用户名当作可随意复用的昵称；修改后旧用户名不能用于新注册。

输入兼容：注册和资料更新仍接受旧 `name` 别名；同时提交 `username` 与 `name` 时必须一致。
输出统一：认证用户、公开用户资料、关注列表、帖子/评论/搜索作者、通知发送者、管理员用户响应将
`name` 改为 `username`；用户名历史项将 `old_name/new_name` 改为
`old_username/new_username`，管理员历史列表将 `name_changes` 改为 `username_changes`。
数据库沿用 `users.name` 及既有审计表列名；其他领域的 `name` 不变。客户端必须更新响应类型。
...
- **类别**：新增端点，非破坏。
- **动机**：用户名是公开身份标识，改名必须保留不可篡改的占用与审计记录；本人和具备用户管理能力的管理员需要能够查询该记录。
- **新增端点**：`GET /api/v2/users/{user_id}/username-history`。
- **可见性**：该端点仅用户本人可读；管理员通过用户取证详情接口查询，普通用户不能读取他人历史。
- **返回语义**：按发生时间倒序返回旧用户名、新用户名与变更时间，不返回内部审核供应商信息。
- **前端影响**：账号设置页可展示本人用户名变更历史；管理员取证页按权限展示指定用户的历史记录。
...
- **类别**：新增端点，非破坏。
- **新增端点**：
  - `POST /api/v2/auth/password-reset-codes`：按邮箱请求重置验证码；统一成功响应，避免枚举账号。
  - `POST /api/v2/auth/password-resets`：提交邮箱、验证码与新密码，成功后撤销全部既有会话但不自动登录。
- **安全语义**：重置验证码与注册验证码按用途隔离，挑战表保存摘要用于校验，邮件任务保存验证码明文用于恢复投递；限时且仅可消费一次。
- **前端影响**：找回密码页先请求验证码，再提交新密码；成功后清除本地登录态并跳转登录页。
...
- **类别**：新增端点，非破坏。
- **动机**：与帖子编辑对齐，PRD 已登记“评论可编辑、全版本留痕、被编辑标识”。
- **新增端点**：`PUT /api/v2/comments/{comment_id}`，仅作者本人可编辑。
- **并发语义**：创建写 revision 1；编辑先对主体行加锁，把编辑后的正文严格追加为
  `max(history.revision)+1`，再推进 `current_revision`。
- **前端影响**：评论需渲染“已编辑”标识；该标识不能由历史是否为空推导。
...
- **类别**：新增端点与既有可见性扩展，非破坏。
- **新增端点**：`POST /api/v2/posts/{post_id}/history/{revision}/restore`，仅作者可把指定旧快照写成新的当前版本；服务端保存回退前版本并继承来源版本的审核结论，不重新送审。
- **删除与恢复语义**：作者自删和管理员下架都保留帖子图片关联、保存删除前当前版本，并对失去全部未删除帖子引用的图片生成转私有与 CDN 刷新意图。管理端可恢复 author、admin、moderation 三种来源；只逆转最新图片审核流水来自 `admin_post_delete` 的封禁。
- **可见性**：软删除帖子仍从信息流、搜索、他人主页与收藏列表排除；作者详情、历史与本人帖子列表可见，本人列表项通过 `is_deleted=true` 标记删除态。

### 注册验证码投递改为持久化任务

- **类别**：行为变更。
- `POST /api/v2/auth/email-verification-codes` 成功表示验证码及投递任务已持久化；供应商暂时
  失败不再回滚挑战并返回 503，而由后台使用同一验证码重试。未配置投递能力仍返回 503。
- 注册与密码重置任务均保存六位验证码明文，消费或失效后清理；HTTP 不返回验证码。

### 用户名改名频率

资料更新接口新增成功改名后连续 30 天（720 小时）的冷却限制，未满 30 天返回 429
`username_change_limited`，客户端提示距上次修改满 30 天后再试。注册设置、规范化后未变化、审核或
占用校验失败不计次；大小写变更和回用旧用户名计次。其他资料更新不受影响。

验证码申请接口成功表示已持久化并受理发送，HTTP 不等待邮件供应商。供应商失败
由后台重试，进程重启后继续处理待发送任务。注册与找回均采用这一响应语义。
