...
func TestAuthReviewRegressions(t *testing.T) {
	...
	t.Run("provider stalls never block registration or reset responses", func(t *testing.T) {
		...
		for _, request := range []struct{ path, email string }{
			{"/api/v2/auth/password-reset-codes", user.Email},
			{"/api/v2/auth/password-reset-codes", "unknown-reset@fdueat.com"},
			{"/api/v2/auth/email-verification-codes", "async-register@fdueat.com"},
			{"/api/v2/auth/email-verification-codes", user.Email},
		} {
			response := make(chan asyncRequestResult, 1)
			go func() {
				status, body, raw, err := performJSONRequest(realEngine, http.MethodPost, request.path, map[string]any{"email": request.email}, "")
